...
public class FileProp {
    ...
    public static String toString(ArrayList<FileProp> files, boolean h){
        SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy HH:mm");
        StringBuilder sb = new StringBuilder("");
        Integer ln = 0;
        for(FileProp i: files){if(i.name.length() > ln) ln = i.name.length();}
        ...
        for(FileProp i: files){
            ...
            if (!h) {
                ...
                if (rights >= 100) {rig+="r"; rights-=100;} else rig+="-";
                if (rights >= 10) {rig+="w"; rights-=10;} else rig+="-";
                if (rights == 1) rig+="x"; else rig+="-";
                while (siz >= 1024) { siz = siz / 1024; x++;}
            }
